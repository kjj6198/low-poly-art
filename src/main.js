    ...
    () => {
      ...
      getImageData(document.querySelector("img"))
        ...
        .then(imageData => {
          ...
          const sobelPoints = sobel(imageData.width, imageData.height)(
            imageData.data
          );
          const sobelData = new ImageData(
            sobelPoints,
            imageData.width,
            imageData.height
          );
          ...
          const randomPoints = randomPickPoints(
            imageData.width,
            imageData.height
          )(Array.from(sobelPoints));

          window.data = makeLowPoly(
            randomPoints,
            Delaunay.triangulate(randomPoints)
          )(ctx, originData);
        })
